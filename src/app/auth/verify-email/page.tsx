  ...
  <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div className="w-full max-w-md">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-[#E0E0E0] overflow-hidden">
        <div className="bg-gradient-to-r from-red-50 to-red-100 border-b border-red-200 p-8 text-center">
          <div className="flex justify-center mb-4">
            <div className="p-4 bg-white rounded-full shadow-lg border-red-200 border-2">
              <Loader2 className="w-16 h-16 text-[#C8102E] animate-spin" />
            </div>
          </div>
          <h2 className="text-2xl font-bold text-black mb-2">Загрузка...</h2>
          <p className="text-[#666666] font-medium">Подготавливаем страницу</p>
        </div>
      </div>
    </div>
  </div>
...
// Основной компонент с логикой верификации
  ...
  const getStatusConfig = () => {
    switch (status) {
      case "loading":
        return {
          icon: Loader2,
          iconClass: "w-16 h-16 text-[#C8102E] animate-spin",
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-[#E0E0E0] overflow-hidden">
          ...
          <div className={`bg-gradient-to-r ${config.bgClass} border-b ${config.borderClass} p-8 text-center`}>
            ...
            <h2 className="text-2xl font-bold text-black mb-2">{config.title}</h2>
            <p className="text-[#666666] font-medium">{config.subtitle}</p>
          </div>

          {/* Content */}
          <div className="p-8">
            <div className="text-center space-y-6">
              <p className="text-[#666666] leading-relaxed">{config.description}</p>

              {/* Loading State */}
              {status === "loading" && (
                <div className="space-y-4">
                  <div className="flex items-center justify-center gap-2 text-[#C8102E]">
                    <Clock className="w-5 h-5" />
                    <span className="text-sm font-medium">Обработка запроса...</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div className="bg-[#C8102E] h-2 rounded-full animate-pulse" style={{ width: "60%" }} />
                  </div>
                </div>
              )}
              ...
              {status === "success" && (
                <div className="space-y-6">
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                    <div className="flex items-center justify-center gap-2 text-[#B00020] mb-2">
                  ...
                  <button
                    ...
                    className="w-full bg-[#C8102E] hover:bg-[#B00020] text-white font-semibold py-3 px-4 rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#C8102E] focus:ring-offset-2"
                  >
              ...
              {status === "error" && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <Link
                      href="/auth/register"
                      className="inline-flex items-center justify-center gap-2 px-4 py-3 bg-[#C8102E] hover:bg-[#B00020] text-white font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#C8102E] focus:ring-offset-2"
                    >
                    ...
                    <Link
                      href="/auth/login"
                      className="inline-flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 hover:bg-gray-200 text-[#666666] font-medium rounded-lg transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
                    >
          ...
          {/* Footer */}
          <div className="bg-gray-50 px-8 py-4 border-t border-[#E0E0E0]">
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-[#666666]">
            Не получили письмо?{" "}
            <button className="text-[#C8102E] hover:text-[#B00020] font-medium">Отправить повторно</button>
          </p>
        </div>
      </div>
    </div>
