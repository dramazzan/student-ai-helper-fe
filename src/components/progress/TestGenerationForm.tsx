...
export const TestGenerationForm: React.FC<TestGenerationFormProps> = ({ weakTopicsCount, onNotification }) => {
  ...
  return (
    <div className="bg-gradient-to-br from-red-50 to-gray-50 rounded-2xl p-6 border border-red-200 shadow-lg">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 bg-gradient-to-br from-[#C8102E] to-[#B00020] rounded-xl flex items-center justify-center">
          <Zap className="w-5 h-5 text-white" />
        </div>
        <div>
          <h3 className="text-xl font-bold text-black">Создать персональный тест</h3>
          <p className="text-sm text-[#666666]">Автоматическая генерация на основе ваших слабых тем</p>
        </div>
      </div>
      {errors.general && (
              ...
              <button
                ...
                className={`p-4 rounded-xl border-2 transition-all duration-200 text-left ${
                  options.difficulty === option.value
                    ...
                    : "border-[#E0E0E0] bg-white hover:border-[#666666] hover:shadow-sm"
                }`}
              >
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full px-4 py-3 border-2 rounded-xl bg-white transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#C8102E] focus:border-[#C8102E] ${
                errors.questionCount ? "border-red-300" : "border-[#E0E0E0]"
              }`}
              min={1}
              max={20}
              placeholder="Введите количество вопросов"
            />
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-[#666666]">
              <span className="text-sm">1-20</span>
            </div>
          </div>
          {errors.questionCount && <p className="text-sm text-red-600 mt-2">{errors.questionCount}</p>}
          <p className="text-xs text-[#666666] mt-2">Рекомендуем 5-10 вопросов для эффективного изучения</p>
        </div>
        {/* Статистика */}
        <div className="bg-white/60 rounded-xl p-4 border border-[#E0E0E0]">
          <div className="flex items-center justify-between text-sm">
            <span className="text-[#666666]">Слабых тем найдено:</span>
            <span className="font-semibold text-black">{weakTopicsCount}</span>
          </div>
        </div>
        {/* Кнопка генерации */}
        <button
